# Analytic flare model copied from the appaloosa code: https://github.com/jradavenport/appaloosa
# Now depreciated use the AltaiPony. We should test AltaiPony's capabilities for finding flares
# for non-TESS/K2/Kepler LCs but for now we only need this model from Davenport et al. 14
...
def flare_model_davenport2014(t, tpeak, fwhm, ampl):
    '''
...
# Lifted from AltaiPony: https://github.com/ekaterinailin/AltaiPony
...
def flare_eqn(t, ampl):
    ...
    eqn = ((1 / 2) * np.sqrt(np.pi) * A * C * f1 * np.exp(-D1 * t + ((B / C) + (D1 * C / 2)) ** 2)
           * special.erfc(((B - t) / C) + (C * D1 / 2))) + ((1 / 2) * np.sqrt(np.pi) * A * C * f2
                                                            * np.exp(
                -D2 * t + ((B / C) + (D2 * C / 2)) ** 2) * special.erfc(((B - t) / C) + (C * D2 / 2)))
    return eqn * ampl


def flare_model_mendoza2022(t, tpeak, fwhm, ampl):
    '''
